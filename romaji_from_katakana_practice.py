...
if "romaji_list_6" not in st.session_state or "katakana_list_6" not in st.session_state:
    romaji_list , katakana_list = get_questions_from_csv(csv_filename)
    st.session_state.romaji_list_6 = romaji_list
    st.session_state.katakana_list_6 = katakana_list

romaji_list = st.session_state.romaji_list_6
katakana_list = st.session_state.katakana_list_6

st.write("### Let's start! \n\n\n\n")
st.write("##### Write the correct romaji (english representation) of the given katakana! \n\n")
...
if(submitted):
    ...
    if(score == len(answers)):
        ...
        for i in incorrect_answer_indexes:
            if(answers[i] != ""):
                st.write("For ", katakana_list[i], " , you wrote ", answers[i])
                st.write("The correct answer is ", romaji_list[i])
                st.write("\n\n")
            else:
                st.write("For ", katakana_list[i], " , you wrote nothing")
                st.write("The correct answer is ", romaji_list[i])
                st.write("\n\n")
        
if(st.button("### Retry")):
    del st.session_state.romaji_list_6
    del st.session_state.katakana_list_6
